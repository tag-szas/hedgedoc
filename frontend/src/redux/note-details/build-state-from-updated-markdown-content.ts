...
import type { RawNoteFrontmatter } from '@hedgedoc/commons/src/note-frontmatter-parser/types'
...
const parseFrontmatter = (frontmatterExtraction: FrontmatterExtractionResult) => {
  return Optional.of(parseRawFrontmatterFromYaml(frontmatterExtraction.rawText))
    ...
    .map((value) => convertRawFrontmatterToNoteFrontmatter(value.value as RawNoteFrontmatter))
